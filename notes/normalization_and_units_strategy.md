...
not a calibrated experimental filling. A central two-flat-band crosswalk is now
generated by `scripts/build_filling_crosswalk.py` and reported in the
Supplemental Material. The mechanism-paper sufficiency of this crosswalk is
checked by `scripts/audit_filling_sufficiency.py`; it is adequate as a BM
central-flat-band counting reference for the current response maps. Any final
comparison to superconducting domes must still compute carrier density from the
full BM spectrum with a declared degeneracy convention or keep the comparison
qualitative.
