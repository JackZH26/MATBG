...
$D_{\rm iso}(\eta)/D_{\rm iso}(0)-1$ & main mechanism observable & absolute stiffness\\
...
\end{verbatim}
and recorded in \code{data/processed/claim\_scope\_audit.csv}.  This audit is
not a numerical validation tool; it checks that the manuscript retains the
boundary statements needed to avoid overinterpreting the finite-band diagnostic
calculation.  Table~\ref{tab:sm_claim_scope} summarizes the current claim
scope.
