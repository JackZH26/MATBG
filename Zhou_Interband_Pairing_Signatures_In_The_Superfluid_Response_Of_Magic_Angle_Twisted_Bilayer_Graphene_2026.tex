...
pairing weight reaches approximately $0.108$ at $\eta=1$ for the
$n_{\rm keep}=6$ truncation.
...
Major-revision stress tests sharpen this statement.  An eight-direction
real-symmetric orbital-pairing family gives positive fixed-Frobenius response
changes in all first-pass family rows, with values between $+6.43\%$ and
$+9.49\%$.  A grid, truncation, and shell matrix for the working direction
gives positive fixed-Frobenius changes in all $120$ tested rows with
$n_{\rm keep}\ge 6$, while also exposing a low-truncation boundary at
$n_{\rm keep}=4$ and $\mu=-4$ meV.  A valley-partner sensitivity test shows
that the positive response is stable in the declared time-reversal-sewn
diagnostic convention, but alternative partner choices are sign-sensitive and
can break the finite-band particle-hole diagnostic.
The fixed raw-gap-amplitude control remains much weaker and sign-sensitive.
We therefore identify the signal as a normalization-conditioned mechanism
signature within a declared finite-band diagnostic convention, rather than an
unconditional or valley-basis-independent enhancement of the absolute
stiffness.
...
response across chemical potential.  The major-revision audits reported below
show that this is not only a single-ansatz observation: it persists across a
tested orbital-pairing family and across the production and expanded
retained-band truncations $n_{\rm keep}=6,8$.
...
\subsection{Major-revision robustness checks}

The first-round review raised three related questions: whether the response is
specific to one orbital direction, whether the sign is stable against grid and
truncation choices, and whether the valley-partner convention is part of the
claim.  Table~\ref{tab:major_revision_evidence} summarizes the additional
audits added in response.

The pairing-family audit scans eight real, symmetric layer-sublattice
directions.  All first-pass fixed-Frobenius rows are positive, with values
between $+6.43\%$ and $+9.49\%$, whereas the fixed-$\Delta_0$ control remains
weaker and sign-sensitive.  This shows that the working
$M_1=\tau_x\sigma_x$ direction is not an isolated successful ansatz in the
current finite-band diagnostic.

The grid, truncation, and shell matrix uses $nk=9,11,13,15$,
$n_{\rm keep}=4,6,8$, $N_{\rm shell}=2,3,4$, and
$\mu=-4,-2,0,2,4$ meV for the working direction.  The fixed-Frobenius response
is positive in all $120$ rows with $n_{\rm keep}\ge 6$ and in all $80$
production-scope rows with $n_{\rm keep}\ge 6$ and $N_{\rm shell}\ge 3$.
This stronger result is deliberately not stated as global truncation
independence: the same matrix exposes a low-truncation boundary at
$n_{\rm keep}=4$ and $\mu=-4$ meV.

The valley-partner sensitivity audit keeps the response formula fixed and
changes only the paired-sector partner vectors.  The production
time-reversal-sewn convention remains positive in all key-point rows and has
zero particle-hole spectrum error.  Alternative partner choices are useful
stress tests, but they are sign-sensitive and show measurable particle-hole
spectrum errors in the present finite-band BdG diagnostic.  Thus the main
claim is robust within the declared \texttt{tr\_sewn} convention, but is not
presently claimed to be independent of valley-partner basis choices.

\begin{table*}[t]
\caption{Major-revision evidence added after the first review.  The table
records the claim-bearing result and the corresponding boundary that remains
in the revised manuscript.}
\label{tab:major_revision_evidence}
\small
\begin{ruledtabular}
\begin{tabular}{p{0.15\textwidth}p{0.21\textwidth}p{0.30\textwidth}p{0.24\textwidth}}
audit & scope & claim-bearing result & boundary retained\\
\hline
pairing family & 8 directions, $nk=7$ & 32/32 positive; $+6.43$ to $+9.49\%$ & fixed-$\Delta_0$: $-1.46$ to $+2.91\%$\\
grid/trunc./shell & $nk=9,11,13,15$; $n_{\rm keep}\ge6$ & 120/120 positive; $+1.07$ to $+13.60\%$ & $n_{\rm keep}=4$, $\mu=-4$ meV can be negative\\
production scope & $n_{\rm keep}\ge6$, $N_{\rm shell}\ge3$ & 80/80 positive; $+4.37$ to $+13.60\%$ & finite-band diagnostic, not continuum value\\
valley partner & \texttt{tr\_sewn}, $n_{\rm keep}=6$ & 24/24 positive; $+4.37$ to $+13.60\%$; PH error $0$ & alternatives are sign/PH sensitive\\
\end{tabular}
\end{ruledtabular}
\end{table*}
...
statement is not normalization independent; the pairing-family audit shows
that the effect is not confined to a single working ansatz; and the
grid/truncation/shell matrix tests that the sign and scale are not artifacts
of the original dense-grid parameters within the production retained-band
scope.
...
controlled time-reversal proxy.  The response-level sensitivity audit shows
that alternative partner choices are not interchangeable with the production
\texttt{tr\_sewn} convention: they can change the response sign and introduce
measurable particle-hole spectrum errors in the present finite-band BdG
diagnostic.  A fully independent two-valley implementation would therefore be
required before claiming valley-basis-independent stiffness physics or
studying valley-asymmetric perturbations.
...
equation is solved.  Fifth, the low-truncation boundary at
$n_{\rm keep}=4$, $\mu=-4$ meV shows that the normalized response is not
globally independent of retained-band cutoff.  Sixth, a UV-complete lattice
calculation would be required for final quantitative comparison with
experiment.
...
The major-revision audits strengthen and delimit this conclusion: a tested
eight-direction orbital-pairing family shows the same positive fixed-Frobenius
diagnostic at the first-pass grid, the production and expanded retained-band
truncations $n_{\rm keep}=6,8$ remain positive across the new
grid/truncation/shell matrix, and the \texttt{tr\_sewn} valley convention is
identified as the PH-consistent production convention rather than an
unexamined gauge choice.
...
normalization-conditioned mechanism signature in the declared finite-band
MATBG superfluid-response diagnostic, not an absolute stiffness prediction
independent of how the gap direction is normalized and not a
valley-basis-independent stiffness theorem.
